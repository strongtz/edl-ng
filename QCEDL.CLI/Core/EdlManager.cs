    ...
    internal class EdlManager : IDisposable
    {
        ...
        public async Task<DeviceMode> DetectCurrentModeAsync(bool forceReconnect = false)
        {
            ...
            try
            {
                ...
                {
                    Logging.Log($"Passive read got {initialReadBuffer.Length} bytes: {BitConverter.ToString(initialReadBuffer).Replace("-", "")}", LogLevel.Debug);
        ...
        private bool FindDeviceLinuxLibUsb()
        {
            ...
            int vidToFind = _globalOptions.Vid ?? 0x05C6;
            int pidToFind = _globalOptions.Pid ?? 0x9008;
            // string serialToFind = _globalOptions.SerialNumber; // If you add a serial number option
            try
            {
                UsbDeviceFinder finder = new UsbDeviceFinder{Vid = vidToFind, Pid = pidToFind};
        ...
        private bool FindDeviceLinuxSerial()
        {
            ...
            string targetVid = (_globalOptions.Vid ?? 0x05C6).ToString("x4");
            string targetPid = (_globalOptions.Pid ?? 0x9008).ToString("x4");
            Logging.Log($"Target VID: {targetVid}, PID: {targetPid}", LogLevel.Debug);
            var potentialTtyPaths = new List<string>();
            try
            {
                string sysTtyPath = "/sys/class/tty";
                ...
                foreach (var dirName in Directory.GetDirectories(sysTtyPath))
                {
                    ...
                    if (ttyName.StartsWith("ttyUSB") || ttyName.StartsWith("ttyACM"))
                    {
                        ...
                        string vidPath = Path.Combine(usbDeviceDir.FullName, "idVendor");
                        string pidPath = Path.Combine(usbDeviceDir.FullName, "idProduct");
                        if (File.Exists(vidPath) && File.Exists(pidPath))
                        {
                            string vid = File.ReadAllText(vidPath).Trim();

                            string pid = File.ReadAllText(pidPath).Trim();
                            ...
                            {
                                string devPath = Path.Combine("/dev", ttyName);
        ...
        private bool FindDeviceWindows()
        {
            ...
            foreach ((string path, string busName, int devInst) deviceInfo in USBExtensions.GetDeviceInfos(COMPortGuid))
            {
                Logging.Log($"Found device via COMPortGuid: {deviceInfo.path} on bus {deviceInfo.busName} (DevInst: {deviceInfo.devInst})", LogLevel.Debug);
                if (IsQualcommEdlDevice(deviceInfo.path, deviceInfo.busName))
                {
                    potentialDevices.Add((deviceInfo.path, deviceInfo.busName, COMPortGuid, deviceInfo.devInst));
                }
            }
            // Search by WinUSBGuid
            foreach ((string path, string busName, int devInst) deviceInfo in USBExtensions.GetDeviceInfos(WinUSBGuid))
            {
                Logging.Log($"Found device via WinUSBGuid: {deviceInfo.path} on bus {deviceInfo.busName} (DevInst: {deviceInfo.devInst})", LogLevel.Debug);
                if (IsQualcommEdlDevice(deviceInfo.path, deviceInfo.busName))
                {
                    potentialDevices.Add((deviceInfo.path, deviceInfo.busName, WinUSBGuid, deviceInfo.devInst));
                }
            }
        ...
        {
            bool isQualcomm = devicePath.Contains("VID_05C6&", StringComparison.OrdinalIgnoreCase);
            ...
            bool isEdl = devicePath.Contains("&PID_9008", StringComparison.OrdinalIgnoreCase);
        ...
        public async Task EnsureFirehoseModeAsync()
        {
            ...
            DeviceMode mode = await DetectCurrentModeAsync(forceReconnect: true);
            switch (mode)
            {
                ...
                case DeviceMode.Sahara:
                    ...
                    {
                        throw new Exception("Device did not re-enumerate in Firehose mode after loader upload, or could not be found.");
                    }
                ...
                default:
                    throw new Exception($"Cannot proceed: Device mode is {mode} or could not be determined.");
            }
        }
        ...
        public void FlushForResponse()
        {
            ...
            try
            {
                while (!GotResponse)
                {
                    Data[] datas = _firehoseClient.GetFirehoseResponseDataPayloads();
            ...
            try
            {
                ...
                if (!_saharaClient.CommandHandshake(_initialSaharaHelloPacket))
                {
                    ...
                    catch (Exception resetEx)
                    {
                        ...
                        throw new Exception("Sahara handshake failed.");
                    }
                }
                else
                {
                ...
                try
                {
                    byte[] sn = _saharaClient.GetSerialNumber();
                    Logging.Log($"Serial Number: {BitConverter.ToString(sn).Replace("-", "")}", LogLevel.Info);
                    if (deviceVersion < 3)
                    {
                        Logging.Log("Sahara version < 3, attempting to get HWID and RKH.", LogLevel.Debug);
                        byte[] hwid = _saharaClient.GetHWID();
                    ...
                    byte[][] rkhs = _saharaClient.GetRKHs();
                    for (int i = 0; i < rkhs.Length; i++)
                    {
                        Logging.Log($"RKH[{i}]: {BitConverter.ToString(rkhs[i]).Replace("-", "")}", LogLevel.Debug);
                    }
                }
                catch (Exception ex)
                {
                ...
                bool success = await Task.Run(() => _saharaClient.LoadProgrammer(_globalOptions.LoaderPath));

                if (!success)
                {
                    throw new Exception("Failed to upload programmer via Sahara.");
                }
        ...
        public async Task ConfigureFirehoseAsync()
        {
            ...
            try
            {
                ...
                StorageType storage = _globalOptions.MemoryType ?? StorageType.UFS;
                ulong maxPayload = _globalOptions.MaxPayloadSize ?? 1048576;

                bool success = await Task.Run(() => _firehoseClient.Configure(storage));
