...
{
    public class QualcommFirehoseXml
    {
        private static string GetEnumXmlName<TEnum>(TEnum value) where TEnum : struct, Enum
        {
            ...
            if (memberInfo != null)
            {
                XmlEnumAttribute? xmlEnumAttribute = memberInfo.GetCustomAttribute<XmlEnumAttribute>();
        ...
        public static string BuildCommandPacket(Data[] dataPayloads)
        {
            ...
            foreach (var data in dataPayloads)
            {
                ...
                if (data.Configure != null)
                {
                    ...
                    if (cfg.ShouldSerializeMemoryName()) configureElement.Add(new XAttribute("MemoryName", GetEnumXmlName(cfg.MemoryName)));
                ...
                else if (data.Read != null)
                {
                    ...
                    if (read.ShouldSerializeStorageType()) readElement.Add(new XAttribute("storage_type", GetEnumXmlName(read.StorageType)));
                ...
                else if (data.Program != null)
                {
                    ...
                    if (prog.ShouldSerializeStorageType()) programElement.Add(new XAttribute("storage_type", GetEnumXmlName(prog.StorageType)));
                ...
                else if (data.Power != null)
                {
                    ...
                    if (pwr.ShouldSerializeValue()) powerElement.Add(new XAttribute("value", GetEnumXmlName(pwr.Value)));
                ...
                else if (data.GetStorageInfo != null)
                {
                    ...
                    if (gsi.ShouldSerializeStorageType()) getStorageInfoElement.Add(new XAttribute("storage_type", GetEnumXmlName(gsi.StorageType)));
        ...
        public static Data[] GetDataPayloads(string commandPacket) // Changed method name for clarity
        {
            ...
            string cleanedXml = commandPacket
            ...
            string wrappedXml = $"<rootWrapper>{cleanedXml}</rootWrapper>";
            ...
            foreach (XElement dataElement in rootElement.Elements("data"))
            {
                ...
                XElement? logElement = dataElement.Element("log");
                if (logElement != null)
                {
                    data.Log = new Log { Value = (string?)logElement.Attribute("value") };
                }

                XElement? responseElement = dataElement.Element("response");
                ...
                XElement? configureElement = dataElement.Element("configure");
                if (configureElement != null)
                {
                    var configure = new Configure();
                    string? memoryNameStr = (string?)configureElement.Attribute("MemoryName");
                ...
                XElement? readElement = dataElement.Element("read");
                if (readElement != null)
                {
                    var read = new Read();
                    string? storageTypeStr = (string?)readElement.Attribute("storage_type");
                ...
                {
                    var power = new Power();
                    string? powerValueStr = (string?)powerElement.Attribute("value");
                ...
                XElement? getStorageInfoElement = dataElement.Element("getstorageinfo");
                if (getStorageInfoElement != null)
                {
                    var gsi = new GetStorageInfo();
                    string? storageTypeStr = (string?)getStorageInfoElement.Attribute("storage_type");
                ...
                XElement? nopElement = dataElement.Element("nop");
                if (nopElement != null)
                {
                    data.Nop = new Nop();
                }
